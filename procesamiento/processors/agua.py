# procesamiento/processors/agua.py
...
import numpy as np
import matplotlib
matplotlib.use("Agg")  # para contenedores / servidores sin GUI
import matplotlib.pyplot as plt

from procesamiento.aux import aux_func_w as auxfunc
...
def _spectralon_path() -> str:
    """
    Resuelve la ruta del archivo de calibración Spectralon dentro del paquete.
    """
...
def _find_measurement_dir(root: str) -> List[str]:
    """
    Busca recursivamente directorios con la estructura:
        Radiometria/Texto Rad CorrPar
    y devuelve la lista de rutas a dichos directorios.
    """
    found: List[str] = []
    for current, dirs, files in os.walk(root):
        # buscamos exactamente la carpeta "Texto Rad CorrPar"
...
def _sorted_txt_files(meas_dir: str) -> List[str]:
    """
    Lista los .txt del directorio de mediciones y los ordena por el número final del nombre.
    """
...
def run(input_dir: str, output_dir: str, config: Dict[str, Any]) -> Dict[str, Any]:
    """
    Procesa mediciones de AGUA en modo stateless.
    - Lee archivos desde 'input_dir' (ZIP previamente extraído).
    - Escribe resultados SOLO dentro de 'output_dir'.
    - Usa parámetros provistos por 'config' (sin tocar archivos fuera del paquete).
    Retorna metadatos/resumen de ejecución.
    """
    # ---------- Configuración ----------
    ...
    # ---------- Spectralon ----------
    # Prioriza el archivo provisto por config (subido por el usuario) y si no, usa el default del paquete.
    ...
    # Preferencia: usar folder_list si coincide; si no, autodetectar
    ...
    if folder_list:
        ...
        # fallback si no se halló ninguna
        ...
        # autodetección global
        for m in _find_measurement_dir(input_dir):
            # usar el nombre de la carpeta superior (campaña) como "folder"
            folder_name = os.path.basename(os.path.dirname(os.path.dirname(m)))  # .../<folder>/Radiometria/Texto...
    ...
    # ---------- Proceso ----------
    produced: List[str] = []
    n_tar = 0  # índice de objetivo/tag progresivo
    for folder, meas_dir in measurement_dirs:
        ...
        for nn in range(n_meas):
            ...
            # Importar matrices de radiancia
            rad_spec, rad_mues, metadata, d_start, d_end = auxfunc.OneMeasurementProcess(
            ...
            # Cielo estimado (mantengo tu criterio: 10% del spec)
            rad_cielo_mean = rad_spec_mean * 0.1

            # Reflectancia agua
            refagua = auxfunc.calculate_refagua(
                rad_mues_mean, rad_cielo_mean, rad_spec_mean, spectralon_reflectance
            )

            # Salidas
            out_folder = os.path.join(output_dir, folder)
            rad_dir = os.path.join(out_folder, "Radiometria", "Texto Radiancia Promedio")
            ref_dir = os.path.join(out_folder, "Radiometria", "Texto Reflectancia Agua")
            auxfunc.check_folders(rad_dir, ref_dir)

            tag = target_list[n_tar] if n_tar < len(target_list) else f"MED_{nn+1:03d}"
            ...
            np.savetxt(root_ref + "_refagua.txt", refagua)
            auxfunc.save_metadata(metadata, root_rad + "_rad_metadata.json")
            auxfunc.save_metadata(metadata, root_ref + "_refagua_metadata.json")
            ...
            if ref_plot == 1:
                fig = auxfunc.reflectance_graph(wavelength, refagua, rad_mues_std, tag)
                fig.savefig(root_ref + "_refagua.png")
                plt.close(fig)

            produced.extend([
                root_rad + "_rad.txt",
                root_rad + "_rad-error.txt",
                root_ref + "_refagua.txt",
                root_rad + "_rad_metadata.json",
                root_ref + "_refagua_metadata.json",
            ])
            if rad_plot == 1:
                produced.append(root_rad + "_rad.png")
            if ref_plot == 1:
                produced.append(root_ref + "_refagua.png")
    ...
    return {
        ...
        "notes": "Procesamiento de agua en modo stateless"
    }


if __name__ == "__main__":
    # Soporte mínimo para ejecución local (opcional):
    #   INPUT_DIR, OUTPUT_DIR, CONFIG_JSON (rutas absolutas)
